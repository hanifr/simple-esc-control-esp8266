...
//Serial.println("incoming: " + topic + " - " + payload); 
  ...
  if (sensorType == sub2)
  {
    ...
    {
      client.publish(sub3, String("ID 2 is Online"));
      ModbusSensors();
...
//    lastMillis = millis();
 //Serial.println("time is good");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    client.publish(sub1, String(DTX));
  }
  else
    WiFi.disconnect();
    WiFi.reconnect();
...
void ModbusSensors(){
  String TxId = "421";
  Serial.println("===================================");
  WLV1 = ULTRASONIC(&wlv1, WLV1_ADDR, WL_REG, 0x01);
  WLV2 = ULTRASONIC(&wlv2, WLV2_ADDR, WL_REG, 0x01);
  //for debugging purpose
  WLV1 = wlv1/1000.0f;   //Serial.println(" WL1: \t" + String(WLV1));
  WLV2 = wlv2/1000.0f;   //Serial.println(" WL2: \t" + String(WLV2));
String  message = "{\"SID\":";
  message += TxId;
  message += ",\"WL1\":";
  message += WLV1;      //Tank A
...
float ULTRASONIC(uint16_t* ws, uint8_t BYTE_ADDR, uint8_t BYTE_REG, uint8_t BYTE_LEN){

  uint16_t temp;
  unsigned long resptime;
  uint8_t frame[FRAMESIZE] = {BYTE_ADDR, FCN_CODE, BYTE_03, BYTE_REG, BYTE_05, BYTE_LEN, 0, 0, 0};
